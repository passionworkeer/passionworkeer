<div align="center">

# Jianjun Wang

**AI-native Product Manager · Agent Engineer · Data Science @ SZTU**

Building AI systems that understand real problems, make decisions, take actions, and verify outcomes.

[![GitHub](https://img.shields.io/badge/GitHub-passionworkeer-181717?style=flat&logo=github)](https://github.com/passionworkeer)
[![Email](https://img.shields.io/badge/Email-17820577270@163.com-EA4335?style=flat&logo=gmail)](mailto:17820577270@163.com)
[![Location](https://img.shields.io/badge/Location-Shenzhen-1677FF?style=flat&logo=googlemaps&logoColor=white)](https://github.com/passionworkeer)
[![Visitors](https://komarev.com/ghpvc/?username=passionworkeer&color=brightgreen&style=flat&label=Profile+Views)](https://github.com/passionworkeer)

</div>

---

## About Me

I work at the intersection of **product, agents, and engineering**.

- AI Product Manager Intern in Shenzhen, exploring enterprise AI, intelligent customer service, workflow automation, and visual AIGC
- Able to move from ambiguous business problems to product requirements, data taxonomies, Agent workflows, evaluation systems, and working prototypes
- Interested in **AI-native products, Agent engineering, FDE-style delivery, multi-agent systems, and reliable long-term memory**
- Currently studying Data Science and Big Data Technology at Shenzhen Technology University

My goal is not only to build an AI feature, but to design a system that can continuously **discover problems → reason → execute → evaluate → improve**.

---

## Current Focus

### Cross-Agent Belief Divergence

> Studying factual belief disagreement between agents as an early-warning signal for silent failures in shared-memory multi-agent systems.

Current research topics include:

- shared-memory consistency and false-success detection
- online belief-divergence measurement
- lightweight pre-action audits
- environment-oracle validation
- adaptations of τ-bench / τ²-bench for memory reliability research

### Enterprise AI Productization

Turning real business workflows into reliable AI systems through:

- requirement discovery and workflow redesign
- intent taxonomy and evaluation-set construction
- RAG / Agent architecture and tool integration
- human-in-the-loop routing and exception handling
- measurable delivery, dashboards, and continuous iteration

---

## Featured Projects

### [yt · Shared Memory Bus](https://github.com/passionworkeer/obsidian-shared-memory-bus)

A local-first shared-memory runtime that lets Claude Code, Codex, Cursor, Claude Desktop, and other MCP clients share structured memory.

- MCP-based cross-tool memory retrieval, writing, and management
- local JSONL event storage, search indexes, and derived Markdown documents
- BM25 and local hash-embedding retrieval with optional remote embedding backends
- split memory services for retrieval, bridge, dream, and management workflows

`Node.js` `Python` `MCP` `SQLite` `BM25` `RAG` `Local-first AI`

### [TwinBuddy](https://github.com/passionworkeer/twinbuddy)

An AI-native companion-matching prototype where digital twins negotiate on behalf of users before presenting a match.

- MING persona distillation framework
- bilateral Agent negotiation with LangGraph
- multi-dimensional compatibility scoring and explainable match reports
- full-stack prototype from product concept to frontend, backend, and Agent workflow

`React` `TypeScript` `FastAPI` `LangGraph` `Multi-Agent`

### [Smart Travel Agent](https://github.com/passionworkeer/gaode_agent)

A travel-planning Agent integrating LLM reasoning, map tools, web search, memory, RAG, and interactive visualization.

- Qwen-based tool-using Agent
- Amap MCP integration for POI search and route planning
- Tavily search and local knowledge retrieval
- personalized memory and interactive map generation

`Python` `LangChain` `Qwen` `MCP` `FAISS` `Gradio`

### AI Code Analysis & Multilingual Repair Benchmark

A university-industry project focused on evaluating and improving AI Coding Agents for cross-file and multilingual code repair.

- AST-aware retrieval and multi-channel RAG fusion
- LoRA-based domain adaptation
- adversarial review and system-level prompt optimization
- continuous scoring and regression-aware evaluation beyond binary pass/fail metrics

`Python` `LoRA` `AST` `RAG` `LLM Evaluation` `AI Coding`

---

## Experience

### AI Product Manager Intern · Banana in `2026.07 – present`

Working on the productization of AI across customer service, enterprise workflows, knowledge systems, and creative production.

- Translate business pain points into product requirements, system workflows, and measurable evaluation plans
- Design intent taxonomies, mapping rules, quality checks, dashboards, and human-handoff strategies for intelligent customer service
- Explore enterprise Agent platforms, workflow orchestration, browser automation, and internal knowledge applications
- Research visual AIGC workflows involving Photoshop plugins, ComfyUI, LoRA, and brand-consistency control
- Coordinate product, engineering, operations, and external vendors through testing and delivery

### AI Application Engineer · Shenzhen Linyuan Technology `2025.10 – 2025.12`

Built a multimodal AI content-generation system for cross-border e-commerce visual workflows.

- Designed an asynchronous architecture with FastAPI, Redis task queues, WebSocket delivery, and failure recovery
- Built a model-routing gateway for heterogeneous LLM and image-generation providers
- Constructed an LLM-as-a-Judge evaluation workflow and optimized prompts with few-shot examples

---

## Recognition

| Award | Role | Year |
|---|---|---:|
| Douyin Hackathon — 2nd Place | Team Lead | 2026 |
| National Math Modeling — Provincial 1st | Team Lead | 2025 |
| Huashu Cup Math Modeling — National 1st | Team Lead | 2025 |
| Zhengda Cup — Provincial 1st | Team Member | 2025 |

---

## Toolbox

**Product & Evaluation**

`Product Discovery` `PRD` `Workflow Design` `Intent Taxonomy` `LLM Evaluation` `Data Analysis` `Dashboard Design`

**Agents & AI Systems**

`LangGraph` `LangChain` `Multi-Agent` `MCP` `RAG` `Shared Memory` `Prompt Engineering` `LoRA` `ComfyUI`

**Engineering**

<div align="center">

[![Skills](https://skillicons.dev/icons?i=python,fastapi,nodejs,react,ts,postgres,redis,docker,git,linux)](https://skillicons.dev)

</div>

---

<div align="center">

**I am looking for opportunities where product judgment and engineering execution are equally important.**

AI Product · Agent Engineer · Applied AI · FDE

</div>
